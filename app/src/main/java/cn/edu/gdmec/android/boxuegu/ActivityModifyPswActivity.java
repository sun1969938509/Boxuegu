...
public class ActivityModifyPswActivity extends Activity implements View.OnClickListener {
    ...
    private void submit() {
        ...
        if (TextUtils.isEmpty(psw)) {
            ...
            Toast.makeText(this,"输入到密码与原始密码不一致",Toast.LENGTH_SHORT).show();
            return;
        }else if(MD5Utils.md5(newPsw).equals(readPsw())){
            Toast.makeText(this,"输入到新密码与原始密码不能一致",Toast.LENGTH_SHORT).show();
            return;
          ...
          Toast.makeText(this, "两次输入到新密码不一致", Toast.LENGTH_SHORT).show();
          return;
        }else {
